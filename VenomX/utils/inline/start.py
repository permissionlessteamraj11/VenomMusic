    ...
    buttons = [
        [
            InlineKeyboardButton(
                text=get_font(_["S_B_1"]),
                url=f"https://t.me/{app.username}?start=help",
            ),
            InlineKeyboardButton(text=get_font(_["S_B_2"]), callback_data="settings_helper"),
        ],
    ]
...
def private_panel(_, BOT_USERNAME, OWNER: Union[bool, int] = None):
    buttons = [
        [InlineKeyboardButton(text=get_font(_["S_B_8"]), callback_data="settings_back_helper")]
    ]
    if SUPPORT_CHANNEL and SUPPORT_GROUP:
        buttons.append(
            [
